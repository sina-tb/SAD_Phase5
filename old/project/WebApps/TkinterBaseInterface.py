...
class TkinterBaseInterface:
    ...
    def package_page_handler(self, list_of_packages):
        ...
        for x in range(len(list_of_packages)):
            tk.Radiobutton(self.pages[1], text=list_of_packages[x], font=("Palatino", 11, "bold"), fg= "blue", bg= "pink" , variable=v, command=ShowChoice,value= x).pack(anchor=tk.W)


        button1 = tk.Button(self.pages[1], text="Submit", font=("Palatino", 11, "bold"), fg= "green", bg= "pink" , command=lambda: self.update_command("fill_prerequisites", {}))
        button1.pack()
